...
#WARNUNG
#
#Ein idiot und anfänger hat diesen coe geschrieben
#Es ist simpel und kompliziert gleichzeitig, inkonseqente kommentarte und
#kein wort dokumentation.
#
#Viel glück hier und viel spaß. Ich empfele zusetzlich ein bisschen notfall Canabis bereitzuhalten
#
####################################################################
version = "a.5"
...
überschriftund = "Arial", 25 ,"underline"
...
def InfoTab():
    InfofF = ttk.Frame(root, borderwidth=2, relief='sunken', \
                            width=418, height=500, padding='80 20 80 20')
    InfofF.propagate(0)
    InfofF.grid(row=2, sticky=(W, N, E, S))
    tabs.add(InfofF, text="Info")

    textF = ttk.Frame(InfofF, height=400)
    textF.pack(side="top")

    InfoL = ttk.Label(textF, text="Info", font=überschriftund)
    InfoL.pack()
    
    AutorenL = ttk.Label(textF, text="Kjell, Berserker", font=subüberschrift)
    AutorenL.pack(pady=20)
    
    VersionL = ttk.Label(textF, text=f"Version: {version}", font=subüberschrift)
    VersionL.pack(pady=20)
    
    DiscordL = ttk.Label(textF, text="Discord", font=subüberschrift, cursor="hand2")
    DiscordL.pack(pady=20)
    DiscordL.bind("<Button-1>", lambda e: ll.callback("https://discord.gg/58HCd8NHuX"))
    
    GitL = ttk.Label(textF, text="Github Projekt", font=subüberschrift, cursor="hand2")
    GitL.pack(pady=20)
    GitL.bind("<Button-1>", lambda e: ll.callback("https://www.example.com/"))
